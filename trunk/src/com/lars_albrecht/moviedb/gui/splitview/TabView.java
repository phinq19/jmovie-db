...
public class TabView extends JTabbedPane {
		...
		for (final Map.Entry<String, Object> tabEntry : tempTab.entrySet()) {
			if (tabEntry.getValue() instanceof JPanel) {
				...
				if (tabEntry.getKey().equalsIgnoreCase("generalmovie")) {
					...
					gbc.gridx = 0;
					final JButton bSaveButton = new JButton(RessourceBundleEx.getInstance().getProperty("application.panel.tabview.button.save.title"));
					...
					gbc.gridx = 1;
					final JButton bRefresh = new JButton(RessourceBundleEx.getInstance().getProperty("application.panel.tabview.button.refresh.title"));
				...
				for (int j = 0; j < Controller.apiScraper.size(); j++) {
					if (Controller.apiScraper.get(j).getTabTitle().equals(tabEntry.getKey())) {
						final JButton bRefreshButton = new JButton(RessourceBundleEx.getInstance().getProperty("application.panel.tabview.apiscraper.button.refresh.title"));
	...
	/**
	 * Returns the given MovieModel "movie" with new informations from the given
	 * apiScraperPlugin.
	 * 
	 * @param apiScraper
	 *            IApiScraperPlugin
	...
	private MovieModel refreshMovieApiScraper(final IApiScraperPlugin apiScraper, MovieModel movie) {
		try {
			...
			if (tempMovie != null) {
				final FieldList fieldlist = Helper.getDBFieldModelFromClass(tempMovie.getClass());
				for (final FieldModel fieldModel : fieldlist) {
					...
					if (tempMovie.get(fieldModel.getField().getName()) != null) {
						...
						if (((oldValue != null) && (newValue != null) && (!oldValue.equals(newValue)) && (JOptionPane.showConfirmDialog(this, String.format(RessourceBundleEx.getInstance()
								.getProperty("application.dialog.refresh.replace.message"), fieldModel.getField().getName(), oldValue, newValue, apiScraper.getPluginName()), RessourceBundleEx
								.getInstance().getProperty("application.dialog.refresh.replace.title"), JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE) == 0))
				...
				String str = null;
				if (((str = JOptionPane.showInputDialog(this, String.format(RessourceBundleEx.getInstance().getProperty("application.dialog.refresh.id.message"), apiScraper.getPluginName()),
						RessourceBundleEx.getInstance().getProperty("application.dialog.refresh.id.title"), JOptionPane.QUESTION_MESSAGE)) != null) && !str.equalsIgnoreCase("")) {
	...
	/**
	 * Checks the filepaths of loaded movies and checks if new movies are in
	 * stored folders and add them automatically.
	 */

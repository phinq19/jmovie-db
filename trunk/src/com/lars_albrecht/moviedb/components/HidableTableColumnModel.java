...
public class HidableTableColumnModel implements TableColumnModel {
	...
	public HidableTableColumnModel(TableColumnModel delegateModel) throws NullPointerException {
		super();
	...
	public void setColumnVisible(int viewIndex, boolean visible) {
		this.setColumnVisible(m_allColumns.get(viewIndex), visible);
	...
	public void setColumnVisible(TableColumn column, boolean visible) {
		if (!visible) {
			...
			for (int idxInvisible = 0; idxInvisible < invisibleColumns; idxInvisible++) {
				final TableColumn visibleColumn = (idxVisible < visibleColumns ? m_delegateModel.getColumn(idxVisible) : null);
				...
				if (testColumn.equals(column)) {
					if (!column.equals(visibleColumn)) {
						...
						m_delegateModel.moveColumn(m_delegateModel.getColumnCount() - 1, idxVisible);
					} // if ! sichtbare Spalte
					return;
				} // if
	...
	public void setAllColumnsVisible() {
		...
		for (int idxColumn = 0; idxColumn < totalSize; idxColumn++) {
			final TableColumn visibleColumn = (idxColumn < m_delegateModel.getColumnCount() ? m_delegateModel.getColumn(idxColumn) : null);
			...
			if (!invisibleColumn.equals(visibleColumn)) {
				...
				m_delegateModel.moveColumn(m_delegateModel.getColumnCount() - 1, idxColumn);
			} // if ungleich
		} // for
	 ...
	 *        <code>= false</code>) oder nur die sichtbaren (<code>= true</code>
	 ...
	 */
	public int getColumnIndex(Object identifier, boolean visibleOnly) throws IllegalArgumentException {
		if (identifier == null)
	...
	public Action[] createColumnActions() {
		...
		for (int i = 0; i < m_allColumns.size(); i++) {
			...
			final Action act = new AbstractAction() {
				...
				{
					this.putValue(SELECTED_KEY, HidableTableColumnModel.this.isColumnVisible(col));
					this.putValue(NAME, col.getHeaderValue());
				}

				@Override
				public void actionPerformed(ActionEvent event) {
					HidableTableColumnModel.this.setColumnVisible(internalColumn, ((Boolean) this.getValue(SELECTED_KEY)).booleanValue());
	 ...
	 */
	@Override
	public void moveColumn(int oldIndex, int newIndex) throws IndexOutOfBoundsException {

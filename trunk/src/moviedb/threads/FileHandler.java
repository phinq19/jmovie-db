...
import moviedb.db.MovieDatabase;
...
public class FileHandler implements Runnable {
	...
	private final MovieDatabase db = null;
	private String dir = null;

	private ArrayList<File> dirs = null;
	private final ArrayList<MovieModel> movies = null;
	...
	private void getMoviesFromFolders (final ArrayList<File> dirs) {
		...
		for (int i = 0; i < dirs.size(); i++) {
			...
			for (final File file : files) {
				if (file.isDirectory() && this.subFolders) {
				...
				if (!file.isDirectory()) {
					...
					this.parseMoviename(filename, tempMovie);
	 ...
	 * @param tempMovie
	 */
	private void parseMoviename (String filename, final MovieModel tempMovie) {
